...
export default class Kformdocuments extends Component {
                ...
                if(documentos[k] === "Póliza de seguro") {
                    documentosGraficos.push(
                        <div className={"containerdoc"}>
                            <input className={"checkdoc"} type={"checkbox"} name={documentos[k]} id={k.toString()}
                                   onChange={Kformdocuments.onChangeCheckBox}/>
    ...
    showDownloadButtons(){
        let buttons = [];
        database.leer_url_documento_campana("7812303", "Currículum vitae").then(result => {
            buttons.push(
    ...
    onChangeFileUploadButton(e){
        try {
            ...
            const task = database.insertar_documento_storage_campana("7812303", tipoDocumento, e.target.files[0]);
            ...
            task.on('state_changed',
                ...
                function complete() { // Lets me know when the file has been uploaded
                    database.insertar_url_nombre_documento_campana("7812303", tipoDocumento, nombreArchivo);
                }
            );
        } catch (err) {
            return;
